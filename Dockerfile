...
# パッケージのインストール
RUN (apt update || apt install -y curl && sudo curl -sSL https://raw.githubusercontent.com/ros/rosdistro/master/ros.key  -o /usr/share/keyrings/ros-archive-keyring.gpg && apt update) && \
    apt upgrade -y && \
    apt install -y \
    bash-completion \
    eog \
